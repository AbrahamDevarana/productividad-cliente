...
import { searchUserAction, updateUserAction } from "../../actions/userActions";
...
const EditUsuarios = () => {
    ...
    const {userSearched} = useSelector( state => state.users )
    ...
    useEffect(() => {
        if(!userSearched){
            dispatch(searchUserAction(slug))
        }
    ...
    const handleSteps = (current) => {
        switch (current) {
            case 0:
                if( (name === null || name.trim() === '') || ( lastName === null || lastName.trim() === '' ) || (secondLastName === null  || secondLastName.trim() === '') || (email === null  || email.trim() === '') || (phone === null  || phone.trim() === '')){
                    notification['error']({
                ...
                dispatch(getEstados())
                handleChangeEstado( town? town.estado_id : 1 )
            ...
            case 1:
                ...
                console.log(town_id);
                if( (street === null || street.trim() === '') || (suburb === null || suburb.trim() === '') || (birth_place === null || birth_place.trim() === '') || (town_id === null || town_id === 0)){
                    notification['error']({
                        message: 'Datos Incompletos',
                ...
                handleFilter(department ? department.area_id : 1)
                setCurrent(current+1)
            break
    
                
            case 3:
                if( (bachelor_degree === null || bachelor_degree.trim() === '') || ( rol_id === null || rol_id === 0) || ( position_id === null || position_id === 0) || (department_id  === null || department_id === 0)){
                    notification['error']({
                        message: 'Datos Incompletos',
    ...
    if(loading || !userSearched || !user) return <Loading/>

    const {name, lastName, secondLastName, email, phone, birth_date, street, suburb, isLeader, birth_place, town_id, bachelor_degree, rol_id, position_id, department_id, admission_date, department, position, town } = user
            ...
            <Box className="px-10">
                ...
                <div className={`pt-20 pb-5 ${current === 0? 'block' : 'hidden'}`}>
                    ...
                    <div className="grid grid-cols-2 gap-10">
                        ...
                        <div className="col-span-1">
                            <DatePicker format={"DD-MM-YYYY"}  placeholder="Fecha de nacimiento" inputName="birth_date" value={moment(birth_date)} onChange={(date, dateString) => (setUser({...user, birth_date: dateString }))} className="w-full border-0 border-b" />
                        </div>
                        <div className="col-span-1">
                    ...
                    <div className="grid grid-cols-2 gap-10">
                        ...
                        <div className="col-span-1">
                            <Select
                                ...
                                className="w-full"
                                defaultValue={ town? town.estado_id : null}
                            >
                                {
                ...
                <h1 className="text-2xl pb-8">Profesional</h1>
                    ...
                    <div className="col-span-1">
                            <Select
                                ...
                                defaultValue={department? department.area_id : null}
                            >
                                {
                        ...
                        <div className="col-span-1"> 
                            <DatePicker format={"DD-MM-YYYY"} placeholder="Fecha de ingreso" inputName="admission_date" value={admission_date ? moment(admission_date) : moment()} onChange={(date, dateString) => (setUser({...user, admission_date:dateString}))} className="w-full border-0 border-b" />
                        </div>
                        <div className="col-span-2">
                            <div className="flex justify-between">
